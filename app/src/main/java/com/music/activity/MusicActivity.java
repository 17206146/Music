...
public class MusicActivity extends AppCompatActivity implements ViewPager.OnPageChangeListener, ViewTreeObserver.OnScrollChangedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        StatusBarCompat.setStatusBarColor(this, Color.parseColor("#5d5d5d"));//设置系统状态栏的颜色
        setContentView(R.layout.activity_music);//加载音乐播放
        ...
        MusicUtil.getInstance().getMediaPlayer().setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
            //因为没有附着在活动中所以就算活动finish掉，还是可以有监听
            @Override
    ...
    public void initLRC(final Song songName, boolean ischang, final int c) {
        ...
        if (ischang){
            HttpUtil.requestSongData(songName.getSong(),1, new Callback() {
                @Override
                ...
                public void onResponse(Call call, Response response) throws IOException {
                    ...
                    String id=MusicFindUtil.parseFindJOSNWithGSON(response).get(0).getId();
                    Log.e(TAG,"id==="+id);
                    HttpUtil.requstNetLrcData(id, new Callback() {
                        @Override
                        public void onFailure(Call call, IOException e) {
            ...
            public boolean onMenuItemClick(MenuItem menuItem) {
                switch (menuItem.getItemId()) {
                    case R.id.menu_item_delete:
                       ...
                       {
                           Toast.makeText(musicService, "Can't delete", Toast.LENGTH_SHORT).show();
                       }
                        break;
